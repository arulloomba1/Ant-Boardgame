...
class Ant(Insect):
    ...
    def add_to(self, place):
        assert place.ant is None or (place.ant.is_container and place.ant.can_contain(self)) or (self.is_container and self.can_contain(place.ant)) , 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        count = 0
        x = self.place
        while count < self.lower_bound and x.entrance:
            x = x.entrance
            count += 1
        while not x.bees and x.is_hive != True:
                x = x.entrance
                count += 1
        if x.is_hive:
            return None
        if self.lower_bound <= count <= self.upper_bound and x.bees:
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            if self.place.bees:
                ...
                for i in temp:
                    if amount + self.damage >= i.health:
...
class BodyguardAnt(ContainerAnt):
    ...
    def store_ant(self, ant):
        return super().store_ant(ant)
    
    def can_contain(self, other):
        return super().can_contain(other)
    
    def remove_ant(self, ant):
        return super().remove_ant(ant)
    # END Problem 8c
    ...
    def action(self, gamestate):
      if self.place.bees:
          ...
          while i != len(self.place.bees):
              new = self.place.bees[i]
              if new.health <= self.damage:
                  i -= 1
                  new.reduce_health(self.damage)
              else:
                  new.reduce_health(self.damage)
              i += 1  
            ...
            while x:
                if x.ant:
                    if x.ant.is_container:
                        if x.ant.ant_contained:
                            x.ant.ant_contained.double()
                    x.ant.double()
                x = x.exit
        super().action(gamestate)
